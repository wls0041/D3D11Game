...
FileStream::FileStream(const std::string & path, const StreamMode & mode)
{
	...
	if (this->flags &Stream_Write) {
		...
		if (out.fail())
		{
			LOG_ERROR_F("Failed to open \"%s\" for writ", path.c_str());
			return;
		}
	}
...
{
	if (this->flags &Stream_Write) out.seekp(n, std::ios::cur);
	else if (this->flags &Stream_Read) in.ignore(n, std::ios::cur);
}
