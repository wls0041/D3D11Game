...
inline void ConstantBuffer::Create(const D3D11_USAGE & usage)
{
	...
	desc.ByteWidth = sizeof(T); //직접 넣어도 되는데 귀찮아서 템플릿으로 만듦
...
{
	D3D11_MAPPED_SUBRESOURCE mapped_resource; //나가서 pData에 적어옴
	auto hr = graphics->GetDeviceContext()->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource); //devicecontext가 gpu와 관련있음.
	assert(SUCCEEDED(hr));
