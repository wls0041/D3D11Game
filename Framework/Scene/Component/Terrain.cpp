...
Terrain::Terrain(Context * context) : context(context) , width(100) , height(100)
{
	graphics = context->GetSubsystem<Graphics>();

	for (uint z = 0; z <= height; z++) //정점을 찍음. 따라서 =붙임
		for (uint x = 0; x <= width; x++) {
			geometry.AddVertex(VertexColor(D3DXVECTOR3(static_cast<float>(x), 0.0f, static_cast<float>(z)), D3DXCOLOR(0, 1, 0, 1)));
			
		}


	for (uint z = 0; z < height; z++) //정점을 이음. =삭제
		for (uint x = 0; x < width; x++) {
...
void Terrain::Render()
{
	...
	world_buffer->BindPipeline(1, ShaderScope::VS); //0번 = 카메라
...
void Terrain::ReadPixel(const std::string * path, std::vector<D3DXCOLOR>& pixels)
{
	...
	auto hr = D3DX11CreateTextureFromFileA(graphics->GetDevice(), path->c_str(), nullptr, nullptr, reinterpret_cast<ID3D11Resource**>(&height_map), nullptr);
	assert(SUCCEEDED(hr));

	D3D11_TEXTURE2D_DESC height_map_desc; //기본적으로 immutable or default로 되어 있기 때문에 cpu가 쓰기 불가
	...
	/////////////////////////////////////////////////////////////
	D3D11_TEXTURE2D_DESC dst_desc; //따라서 복사본을 만들고 새로 플래그를 줌
	...
	dst_desc.CPUAccessFlags = D3D11_CPU_ACCESS_READ | D3D11_CPU_ACCESS_WRITE;

	ID3D11Texture2D *dst_texture = nullptr;
	hr = graphics->GetDevice()->CreateTexture2D(&dst_desc, nullptr, &dst_texture);

	hr = D3DX11LoadTextureFromTexture(graphics->GetDeviceContext(), height_map, nullptr, dst_texture); //texture로부터 로딩
	assert(SUCCEEDED(hr));
}
