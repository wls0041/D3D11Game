...
auto TextureImporter::Load(const std::string & path, Texture * texture) -> const bool
{
	if (!texture)
	{
...
void TextureImporter::GenerateMipmaps(FIBITMAP * bitmap, Texture * texture, uint width, uint height, uint channels)
{
	if (!texture)
	{
...
auto TextureImporter::ConvertTo32Bits(FIBITMAP * bitmap) -> struct FIBITMAP *
{
	...
	if (!bitmap)
	{
		Log::Error_f("ImageImporter::ConvertTo32Bits : Failed (%d bpp, %d channels).", FreeImage_GetBPP(previousBitmap), ComputeChannelCount(previousBitmap));
		return nullptr;
	}
